...
//Active patterns
    module Patterns =
        ...
        let (|Monomial|_|) (u : Expression) =
            ...
            | BinaryOp (a,pow,(Number (Integer b)),set) when //MON-3
                pow = DefaultValues.pow &&
                b > 1I -> Some u 
                
            | NaryOp (product, [a;b],set) when product = DefaultValues.product && //MON-4            
                match a, b with                 
                | Number n, Symbol s -> true                
                | Number n, BinaryOp (Symbol s,pow,(Number (Integer b)),set) when pow = DefaultValues.pow && b > 1I -> true
                | _ -> false
                -> Some u
            | _-> None
        ...
        let rec (|AlternateGeneralMonomial|_|) (x : Expression) = function        
            | BinaryOp (a,pow,(Number (Integer b)),set) when pow = DefaultValues.pow && b > 1I -> //GMEALT-2
                 Some (BinaryOp (a,pow,(Number (Integer b)),set))
            
            | NaryOp (p,a,set) when p = DefaultValues.product -> //GMEALT-3
                let test = List.forall (fun a ->
                    (match x with 
                     | AlternateGeneralMonomial x a -> true
                     | NaryOp(sum,s,set) when sum = DefaultValues.sum -> true 
                     | _ -> false) = true) a
                match test with
                | true -> Some (NaryOp (product, a,set))
                | false -> None        
            | NaryOp(s,a,set) when s = DefaultValues.sum -> None //GMEALT-4
            | a when a = x -> Some a //GMEALT-1            
            | _ -> Some x //GMEALT-4
 
        let (|AlternateGeneralPolynomial|_|) (x : Expression) = function
            | NaryOp (s, a, set) when s = DefaultValues.sum -> //GPEALT-2
                match List.forall (fun a' -> (match a' with | AlternateGeneralMonomial x a' -> true | _ -> false) = true) a with
                | true -> Some (NaryOp (sum,a,set))
                | false -> None
        ...
        let rec (|GExpression|_|) (alpha : Expression) = function
            ...
            | NaryOp (op,a,set) as naryOp when op = DefaultValues.sum || op = DefaultValues.product -> //G-4 and G-5
                let test = List.forall (fun a' ->
                    (match a' with 
                ...
                | true -> Some naryOp
                | false -> None                  
            | BinaryOp (GExpression alpha a,toThePowerOf,(Number (Integer b)),set) as B when toThePowerOf = DefaultValues.pow -> Some B //G-6
            | _-> None
        ...
        let rec (|RSIN|_|) = function
            ...
            | NaryOp (op, a,set) as naryOp when op = DefaultValues.sum || op = DefaultValues.product -> //RS-IN-4 and 5
                let test = List.forall (fun a' ->
                    (match a' with 
                     | RSIN a' -> true                     
                     | _ -> false) = true) a
                ...
                | true -> Some naryOp
                | false -> None                  
            | BinaryOp (RSIN a,toThePowerOf,(Number (Integer b)),set) as B when toThePowerOf = DefaultValues.pow -> Some B //RS-IN-6
            | _-> None
...
//Check functions
    ...
    module Check =
        ...
        let expressionListContainsSum expList = 
            List.exists (fun x -> 
            match x with 
            | NaryOp(sum, xList,set) when sum = DefaultValues.sum -> true
            | _ -> false ) expList

        let expressionListContainsNegativePower expList = 
            List.exists (fun x -> 
            match x with 
            | BinaryOp(a,toThePowerOf,Number(Integer b),set) when b < 0I -> true
            | _ -> false ) expList
        ...
        let sumContainsConstantTerm x = 
            match x with
            | NaryOp(sum, xList,set) when sum = DefaultValues.sum -> 
                List.exists (fun x -> 
                match x with 
                | Number n -> true
                | _ -> false ) xList
...
//Variables of a polynomial  
    ...
    module Variables =
        ...
        let rec private _variablesOf acc u = 
            match u with
            | Number n -> acc //Var-1
            | BinaryOp(a,toThePowerOf,Number(Integer b),set) when b > 1I && toThePowerOf = DefaultValues.pow -> a::acc //Var-2
            | NaryOp(product,aList,set) when product = DefaultValues.product -> 
                List.collect (fun x -> //Var-4
                    match x with 
                    | NaryOp(sum,bList,set) when sum = DefaultValues.sum -> NaryOp(sum,bList,set)::acc
                    | _ -> (_variablesOf acc x)) aList
            | NaryOp(sum,aList,set) when sum = DefaultValues.sum -> List.collect (fun x -> _variablesOf acc x) aList //Var-3
            | _ -> u::acc //Var-5
            |> Seq.distinct
            |> Seq.toList
